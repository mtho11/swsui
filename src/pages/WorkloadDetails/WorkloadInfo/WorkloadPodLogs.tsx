...
export default class WorkloadPodLogs extends React.Component<WorkloadPodLogsProps, WorkloadPodLogsState> {
    ...
    return (
      <div className={proxyLogsDiv}>
        <Toolbar className={toolbar}>
          ...
          <ToolbarGroup className={toolbarRight}>
            <ToolbarItem>
              <Tooltip key="copy_proxy_logs" position="top" content="Copy app logs to clipboard">
                <CopyToClipboard
  ...
  private copyAppLogCallback = (_text: string, result: boolean) => {
    console.log('CopyCallback: ' + result);
    console.log(_text);
    this.appLogsRef.current.select();
    // this.setState({ copyStatus: result ? CopyStatus.COPIED : CopyStatus.NOT_COPIED });
  };

  private copyProxyLogCallback = (_text: string, result: boolean) => {
    console.log('CopyCallback: ' + result);
    console.log(_text);
